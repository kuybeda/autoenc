...
class DualEncoder(nn.Module):
    def __init__(self, nz):
        super().__init__()
        self.rgb    = nn.DataParallel(Encoder(nz).cuda())
        self.fir    = nn.DataParallel(Encoder(nz).cuda())

class DualGenerator(nn.Module):
    def __init__(self, nz):
        super().__init__()
        self.rgb    = nn.DataParallel(Generator(nz).cuda())
        self.fir    = nn.DataParallel(Generator(nz).cuda())

class DualCritic(nn.Module):
    def __init__(self, nz):
        super().__init__()
        self.rgb    = nn.DataParallel(Critic(nz).cuda())
        self.fir    = nn.DataParallel(Critic(nz).cuda())
...
class Autoencoder(object):
    ...
    def __call__(self,x,phase,alpha):
        z = self.enc(x, phase, alpha)
        return self.gen(z, phase, alpha)
...
class MixCoder(object):
    def __init__(self, enc1, enc2, gen1, gen2, style_prop):
        super().__init__()
        self.enc1 = enc1
        self.enc2 = enc2
        self.gen1 = gen1
    ...
    def cross_combine(self,z1,z2):
        z1_style, _ = self.split_channels(z1)
        ...
        return torch.cat((z1_style,z2_struct),dim=1)

    def __call__(self, x1, phase, alpha):
        z1      = self.enc1(x1, phase, alpha)
        # combine with random style input
        z2      = self.rand_combine(z1)
        x2_hat  = self.gen2(z2, phase, alpha)
        z2_hat  = self.enc2(x2_hat, phase, alpha)
        # make a style shortcut from z1 to z1_hat
        z1_hat  = self.cross_combine(z1,z2_hat)
        x1_hat  = self.gen1(z1_hat, phase, alpha)
        return x2_hat, x1_hat

class DualMixCoder(object):
    def __init__(self, dualenc, dualgen, style_prop):
        super().__init__()
...
class Model(OptimModule):
    ...
    def autoenc_loss(autoenc, crt, x, phase, alpha):
        ...
        return [loss_x, alpha*loss_crt], x_fake.detach()

    @staticmethod
    def mix_loss(mixcod, crt1, crt2, x1, x2, phase, alpha):
        x1_x2, x1_x1 = mixcod(x1, phase, alpha)
    ...
    def update(self, batch, phase, alpha):
        ...
        loss_grad_auto_rgb = Model.crt_grad_penalty(self.crt.rgb, rgb, auto_fake_rgb, phase, alpha, gnorm_auto_rgb)
        ...
        # equalize graqient between real and fake samples
        loss_grad_auto_fir = Model.crt_grad_penalty(self.crt.fir, fir, auto_fake_fir, phase, alpha, gnorm_auto_fir)
        ...
        loss_grad_mix_rgb = Model.crt_grad_penalty(self.crt.rgb, rgb, mix_fake_rgb, phase, alpha, gnorm_mix_rgb)
        ...
        loss_grad_mix_fir = Model.crt_grad_penalty(self.crt.fir, fir, mix_fake_fir, phase, alpha, gnorm_mix_fir)
    ...
    def dry_run(self, batch, phase, alpha):
        ...
        rgb_fir, rgb_rgb = self.mixcod.rgb(rgb,phase,alpha)
